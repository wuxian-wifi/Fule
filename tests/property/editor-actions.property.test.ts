 ...
 * 撤销后应等于原始内容且 isDirty 为 false。
 *
 * 测试通过 EditorStore 的 updateTabContent / markTabClean 直接验证
 * 接受和撤销的核心逻辑，与 DiffActionBar 组件使用的 store 操作一致。
 */
describe('Property 3: AI 代码变更接受/撤销往返正确性', () => {
    ...
    fc.assert(
      fc.property(
        ...
        ({ original, modified }) => {
          ...
          const tabId = '/test/editor-action.ts'

          // 打开标签页并设置原始内容
          useEditorStore.getState().openTab({
            id: tabId,
            filePath: tabId,
            fileName: 'editor-action.ts',
            content: original,
            language: 'typescript',
          })

          // 模拟"接受" AI 变更：用修改内容更新标签页（与 DiffActionBar 的 handleAccept 一致）
  ...
  it('撤销 AI 变更后，标签页内容应等于原始内容且 isDirty 为 false', () => {
    /**
     * Validates: Requirements 2.3
     */
    fc.assert(
      fc.property(
        ...
        ({ original, modified }) => {
          ...
          // 模拟"撤销"：恢复原始内容并标记为干净（与 DiffActionBar 的 handleRevert 一致）
  ...
  it('接受后再撤销的完整往返，内容应回到原始状态且标记为干净', () => {
    /**
     * Validates: Requirements 2.3
     */
    fc.assert(
      fc.property(
        ...
        ({ original, modified }) => {
          ...
          // 接受变更 → 内容为 modified，isDirty 为 true
          useEditorStore.getState().updateTabContent(tabId, modified)
          ...
          // 撤销变更 → 内容回到 original，isDirty 为 false
          useEditorStore.getState().updateTabContent(tabId, original)
          useEditorStore.getState().markTabClean(tabId)
